...
def calcularpi_kernel(rng_states,  iteraciones, out):
    ...
    # la fracción de ellos que cae dentro del circulo unutario
    cae_adentro = 0
    for i in range(iteraciones):
        x = xoroshiro128p_uniform_float64(rng_states, ii)
        y = xoroshito128p_uniform_float64(rng_states, ii)
        if x**2 +y**2 <= 1,0:
            cae_adentro += 1
...
out = np.zero(hilosporbloque*bloques, dtype=np.float64)
